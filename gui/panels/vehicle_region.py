...
class VehicleRegion:
    def __init__(self, parent):
        ...
        self.params_frame = ttk.LabelFrame(
            self.container_frame, text="Vehicle Parameters"
        )
        ...
        self.params_scrollbar = ttk.Scrollbar(
            self.params_frame, orient=tk.VERTICAL, command=self.params_canvas.yview
        )
        ...
        self.params_canvas.create_window(
            (0, 0), window=self.params_inner_frame, anchor="nw"
        )
        ...
        self.ic_scrollbar = ttk.Scrollbar(
            self.ic_frame, orient=tk.VERTICAL, command=self.ic_canvas.yview
        )
        self.ic_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.ic_canvas.configure(yscrollcommand=self.ic_scrollbar.set)
        self.ic_canvas.bind(
            "<Configure>",
            lambda e: self.ic_canvas.configure(scrollregion=self.ic_canvas.bbox("all")),
        )
    ...
    def get_parameters(self):
        return {
            key: float(param.get_value()) for key, param in self.current_params.items()
        }
